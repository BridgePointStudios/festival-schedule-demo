...
Router.route('/getPDF', function() {
      ...
      var evnt = Events.findOne("karN8kgxXPxq9bCxn");
      console.log (evnt);
      ...
      doc.text('Booking Agreement');
      doc.fontSize(12);
      doc.text('This contract entered into on the date of ' + pdfCreatedDate + ' is for the personal services of the listed performer for the performance described below. It indicates that the undersigned purchaser, ' + venue.venueName + ', the undersigned performer, ' + band.bandName + ' and Muscle Shoals Artists Unlimited (hereafter referred to as AGENT) agree and contract as follows: ');
      doc.text(' ');
      doc.text('1. Name of Musician or Band: ' + band.bandName);
      doc.text('2. Number of Musicians: ' + band.bandNumberMusicians);
      doc.text('3. Name and Address of Place of Performance:' + venue.venueName + ', ' + venue.venueAddress.street + (venue.venueAddress.street2 != '' ? venue.venueAddress.street2 + ', ' : '') + venue.venueAddress.city + ', ' + venue.venueAddress.state + ', ' + venue.venueAddress.postalCode);
      doc.text('4. Date and Time of Performance: ' + moment(evnt.eventStartTime).format('MMMM Do YYYY, h:mm a') + 'until' + moment(evnt.eventStopTime).format('h:mm a'));
      doc.text('5. Wage and Deposit Agreed Upon: ' + evnt.eventBandPayment);
      doc.text('6. At the end of the performance, payment is to be made to the following (in either U.S. currency or certified check): ' + band.bandContactName);
      doc.text('7. Additional Terms:');
      doc.text('PA: ' + evnt.eventPaProvided);
      doc.text('Food & Beverage Tab: ' + evnt.eventTabProvided);
      doc.text('Merchandise: ' + evnt.eventMerch);
      doc.text('Lodging: ' + evnt.eventLodging);
      doc.text(' ');
      ...
      doc.text('In case of breach of this contract by Employer, the Employer agrees to pay the amount stated in Section 6 as mitigated damages, plus reasonable attorney\'s fees, court costs, and legal interest.');
      doc.text(' ');
      doc.text('The persons signing for Employer and Musician(s) agree to be personally, jointly and severally liable for the terms of this contract.');
      doc.text(' ');
      doc.text('For MSAU:');
      doc.text(' ');
